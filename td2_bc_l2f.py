...
def gather_buffer(model, name='l2f_controller_buffer', size = 10000, step_len = 501):
    buffer = CustomBuffer(size=size*5)
    n_rollouts = size 
    for _ in tqdm(range(n_rollouts), desc="Gathering buffer"):
        ...
        buffer.add({"obs": np.array(obs_lst), "act": np.array(action_lst), "rew": np.array(rewards_lst), "done": np.array(dones_lst), "obs_next": np.array(obs_next_lst)})
