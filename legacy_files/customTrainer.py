...
class BaseTrainer(ABC):
    ...
    def training_step(self) -> tuple[CollectStatsBase, TrainingStats | None, bool]:
        ...
        with policy_within_training_step(self.policy):
            ...
            if self.train_collector is not None:
                if hasattr(self.train_collector.policy.actor.preprocess, "epoch"):
                    if self.train_collector.policy.actor.preprocess.epoch != self.epoch:
                        self.train_collector.policy.actor.preprocess.epoch = self.epoch
...
class OffpolicyTrainer(BaseTrainer):
    ...
    def policy_update_fn(
        ...
        # print(f"performing policy update, n_collected_steps={n_collected_steps}, n_gradient_steps={n_gradient_steps}")
       ...
       #  print(f"policy update took {time.time() - start} seconds")
        return update_stat
