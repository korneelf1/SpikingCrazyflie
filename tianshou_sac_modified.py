...
class SACPolicy(DDPGPolicy[TSACTrainingStats], Generic[TSACTrainingStats]):  # type: ignore[type-var]
    ...
    def forward(  # type: ignore
        ...
        **kwargs: Any,
        ...
        # batch = Batch.to_torch(batch, dtype=torch.float32,device=self.device)
    ...
    def learn(self, batch: RolloutBatchProtocol, *args: Any, **kwargs: Any) -> TSACTrainingStats:  # type: ignore
        # print(f'Starting learn step...')
        # start = time.time()
        # mem1 = torch.cuda.memory_allocated()
        batch = Batch.to_torch(batch, dtype=torch.float32,device=self.device)
        # print(f'After moving batch to GPU: {torch.cuda.memory_allocated()} bytes')
        # print(f'Difference: {(torch.cuda.memory_allocated() - mem1)/1e3} kilobytes')
        ...
        self.sync_weight()
        # print(f'Learn step took {time.time() - start} seconds')
