...
class TD3BC_Online:
    ...
    def run(self):
        ...
        for i in range(50,450,25):
            self.learn(epoch=cur_epoch, end_epoch=cur_epoch+25)
            n_epochs_tot+=25
        ...
        while n_epochs_tot<500:
            self.learn(epoch=cur_epoch, end_epoch=cur_epoch+50)
            n_epochs_tot+=50
            cur_epoch+=50
            if self.curriculum:
                self.env.update_curriculum()
            self.gather_buffer(jump_start_len=50)
SIGMA_MIN = 1e-3
SIGMA_MAX = .2
...
if __name__ == "__main__":
    ...
    def get_args() -> argparse.Namespace:
        ...
        parser.add_argument("--bc-factor", type=float, default=0.75, help="Behavioral cloning factor")
