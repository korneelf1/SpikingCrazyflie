...
class TD3BC:
    ...
    # def recompute_rewards(self, batch):
    #     '''Recompute the rewards based on the current reward parameters'''
    #     rewards = batch.rew
    #     observations = batch.obs
    #     actions = batch.act
        
    #     rewards = np.zeros_like(rewards.detach().cpu())
    #     for t in range(len(rewards)):
    #         rewards[:,t] = self.env._reward(observations[:,t].detach().cpu(), actions[:,t].detach().cpu())
    #     batch.rew = rewards

    def recompute_rewards(self, batch):
        """Recompute rewards for the batch efficiently."""
        # Assume rewards are shape (batch, T)
        batch_size, seq_len = batch.rew.shape

        # Move all observations and actions to CPU once
        obs_cpu = batch.obs.detach().cpu().numpy()
        act_cpu = batch.act.detach().cpu().numpy()

        # Preallocate reward array
        rewards = np.zeros((batch_size, seq_len), dtype=np.float32)

        # Compute rewards per timestep (vectorized over batch)
