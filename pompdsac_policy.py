...
class POmpdSACTrainingStats(TrainingStats):
    actor_loss: float
    state_prediction_loss: float
    critic1_loss: float
    critic2_loss: float
...
POmpdTSACTrainingStats = TypeVar("POmpdTSACTrainingStats", bound=POmpdSACTrainingStats)


# TODO: the type ignore here is needed b/c the hierarchy is actually broken! Should reconsider the inheritance structure.
class POMPDSACPolicy(DDPGPolicy[POmpdTSACTrainingStats], Generic[POmpdTSACTrainingStats]):  # type: ignore[type-var]
    ...
    :param beta: coefficient for velocity and orientation prediction loss. An additional task for POMPDs.
    ...
    def __init__(
        ...
        *,
        ...
        tau: float = 0.005,
        gamma: float = 0.99,
        alpha: float | tuple[float, torch.Tensor, torch.optim.Optimizer] = 0.0,
        beta: float = 0.1,
    ...
    def forward(  # type: ignore
        ...
        **kwargs: Any,
        ...
        Assumes that batch.obs is a batch of observations.
        :param batch: a batch of observations.
        :param state: the hidden state of the actor network.
        :param kwargs: additional keyword arguments.'''
        (loc_B, scale_B), hidden_BH, vel_orient_prediction = self.actor(batch.obs, state=state, info=batch.info)
        ...
        result = Batch(
            ...
            vel_orient_prediction=vel_orient_prediction,
            act=squashed_action,
            state=hidden_BH,
            dist=dist,
            log_prob=log_prob,
    ...
    def learn(self, batch: RolloutBatchProtocol, *args: Any, **kwargs: Any) -> POmpdTSACTrainingStats:  # type: ignore
        # critic 1&2
        ...
        # velocity and orientation prediction loss
        # loss is next real veloicity and orientation and predicted velocity and orientation
        # print(obs_result.vel_orient_prediction.shape, batch.obs_next[:,9:].shape)
        vel_orient_prediction_loss = torch.nn.MSELoss()(obs_result.vel_orient_prediction, torch.tensor(batch.obs_next[:,9:])).mean()

        actor_loss = (
            self.alpha * obs_result.log_prob.flatten() - torch.min(current_q1a, current_q2a)
        ).mean()

        total_loss = actor_loss + self.beta * vel_orient_prediction_loss
        ...
        return POmpdSACTrainingStats(  # type: ignore[return-value]
            actor_loss=actor_loss.item(),
            state_prediction_loss=vel_orient_prediction_loss.item(),
