...
from training.td3bc_online import TD3BC_Online
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bc-val", type=float, default=0.2, help="BC coefficient value")
    parser.add_argument("--bc-factor", type=float, default=0.95, help="BC factor for decay")
    parser.add_argument("--jumpstart", action='store_true', help="Enable jumpstart phase")
...
if __name__ == "__main__":
    ...
    wandb_args = {"spiking":True, 'Slope': args.slope,'Schedule': args.slope_schedule, 'Algo':'TD3BC_Online', 'fast_learning':False, 'scheduling_order':args.scheduling_order, 'curriculum':args.curriculum}
    ...
    print("BC value:",args.bc_val)
    print("BC factor:",args.bc_factor)
    print("Jumpstart:",args.jumpstart)
    wandb.config.update({'slope':args.slope, 'slope_schedule':args.slope_schedule,'scheduling_order':args.scheduling_order,'hidden_sizes':args.hidden_sizes, 'policy_noise':args.policy_noise, 'curriculum':args.curriculum, 'bc_val':args.bc_val, 'bc_factor':args.bc_factor, 'jumpstart':args.jumpstart})
    ...
    # Create a simple controller for jumpstart
    controller = Wrapper(spiking_module, size=args.hidden_sizes[-1]).to(device)
    
    # prepare the TD3BC_Online  
    td3bc_online = TD3BC_Online(env, model, optimizer, critic1, critic1_optimizer, critic2, critic2_optimizer, buffer, batch_size=50, device=device, controller=controller, curriculum=args.curriculum, bc_val=args.bc_val, bc_factor=args.bc_factor)
    # learn the model
    td3bc_online.run(jumpstart=args.jumpstart)
    wandb.run.finish()
